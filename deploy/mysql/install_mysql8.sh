...
# [2] 초기 SQL ConfigMap 생성
echo "[2/6] 초기 SQL(database_dump.sql)로 ConfigMap 생성"
kubectl create configmap mysql-initdb \
  --from-file=init.sql=./init-sql/database_dump.sql \
...
echo "    또는 내부에서는: $DB_HOST.production.svc.cluster.local:3306"
echo ""
kubectl get svc -n $NAMESPACE

# [7] Pod 준비 상태 대기 후 노드 정보 출력
echo "⏳ MySQL Pod가 준비될 때까지 대기 중..."
kubectl wait --for=condition=Ready pod -l app.kubernetes.io/instance=$DB_HOST -n $NAMESPACE --timeout=60s || echo "⚠️ Pod가 준비되지 않았습니다."
...
echo "🔗 MySQL Pod IP: $(kubectl get pod $POD_NAME -n $NAMESPACE -o jsonpath="{.status.podIP}")"